...
import { Heart, Loader2, AlertTriangle, Download, ArrowLeft } from 'lucide-react';
import { getSharedHeartAssessment } from '../services/api';
import { ROUTES } from '../config/constants';

export default function SharedHeart({ language }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await getSharedHeartAssessment(token);
        setData(result);
      } catch (err) {
        setError(err.message || (isAr ? 'تقييم القلب غير موجود أو الرابط منتهي الصلاحية.' : isTr ? 'Kalp değerlendirmesi bulunamadı.' : 'Heart assessment not found or link expired.'));
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handlePrint = () => {
    if (!data) return;
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Heart Assessment Report</title>
<style>body{font-family:Arial,sans-serif;max-width:700px;margin:40px auto;padding:20px;color:#333}
h1{color:#db2777;border-bottom:2px solid #db2777;padding-bottom:8px}
  ...
  if (error) {
    return (
      <div className="max-w-lg mx-auto px-6 py-32 text-center">
        ...
        <h1 className="text-xl font-bold text-white mb-2">{isAr ? 'لم يتم العثور على تقييم القلب' : isTr ? 'Kalp Değerlendirmesi Bulunamadı' : 'Heart Assessment Not Found'}</h1>
        <p className="text-gray-400 text-sm mb-6">{error}</p>
        <Link to={ROUTES.HOME} className="inline-flex items-center gap-2 text-sm text-pink-400 hover:text-pink-300">
      ...
      <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/[0.08]">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">{isAr ? 'تقييم قلب مشترك' : isTr ? 'Paylaşılan Kalp Değerlendirmesi' : 'Shared Heart Assessment'}</h1>
