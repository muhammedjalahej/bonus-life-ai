...
import { Activity, ArrowLeft, Download, Share2, Loader2 } from 'lucide-react';
import { ROUTES } from '../config/constants';
import { useAuth } from '../context/AuthContext';
import apiService from '../services/api';
import { buildAndDownloadSignedPDF } from '../utils/assessmentPdf';
...
export default function AssessmentReportPage({ language }) {
  ...
  const handleShare = async () => {
    ...
    try {
      const result = await apiService.shareAssessment(assessment.id);
  ...
  const backToAssessments = () => {
    navigate(`${ROUTES.DASHBOARD}?tab=assessments`);
  };
    ...
    return (
      <div className="max-w-2xl mx-auto px-6 pt-32 pb-24">
        <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/[0.08] text-center">
          <p className="text-gray-400 mb-4">{isAr ? 'التقييم غير موجود.' : isTr ? 'Değerlendirme bulunamadı.' : 'Assessment not found.'}</p>
          <Link
          ...
          >
            ...
            {isAr ? 'العودة إلى تقييماتي' : isTr ? 'Değerlendirmelerime dön' : 'Back to My Assessments'}
          </Link>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/[0.06]">
          ...
          <div>
            <h1 className="text-lg font-bold text-white">
              {isAr ? 'تقرير التقييم' : isTr ? 'Değerlendirme Raporu' : 'Assessment Report'}
            </h1>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
              <p className="text-xs text-gray-500 mb-1">{isAr ? 'مستوى الخطر' : isTr ? 'Risk Seviyesi' : 'Risk Level'}</p>
            ...
            <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
              <p className="text-xs text-gray-500 mb-1">{isAr ? 'الاحتمالية' : isTr ? 'Olasılık' : 'Probability'}</p>
          ...
          {shareLink?.id === assessment.id && (
            <div className="p-2 rounded-lg bg-violet-500/10 border border-violet-500/20">
              <p className="text-xs text-violet-400">{isAr ? 'تم نسخ الرابط:' : isTr ? 'Link kopyalandı:' : 'Link copied:'}</p>
