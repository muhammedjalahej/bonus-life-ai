    ...
    return (
      <div className="max-w-2xl mx-auto px-6 pt-32 pb-24">
        <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/[0.08] text-center">
          <p className="text-gray-400 mb-4">{isAr ? 'تقييم القلب غير موجود.' : isTr ? 'Kalp değerlendirmesi bulunamadı.' : 'Heart assessment not found.'}</p>
          <Link
          ...
          >
            ...
            {isAr ? 'العودة إلى تقييمات القلب الخاصة بي' : isTr ? 'Kalp değerlendirmelerime dön' : 'Back to My Heart Assessments'}
          </Link>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/[0.06]">
          ...
          <div>
            <h1 className="text-lg font-bold text-white">
              {isAr ? 'تقرير تقييم القلب' : isTr ? 'Kalp Değerlendirme Raporu' : 'Heart Assessment Report'}
            </h1>
        ...
        <div className="space-y-4">
          ...
          {shareLink?.id === assessment.id && (
            <div className="p-2 rounded-lg bg-pink-500/10 border border-pink-500/20">
              <p className="text-xs text-pink-400">{isAr ? 'تم نسخ الرابط:' : isTr ? 'Link kopyalandı:' : 'Link copied:'}</p>
