...
import { Droplets, ArrowLeft, Download, Share2, Loader2 } from 'lucide-react';
import { ROUTES } from '../config/constants';
import { useAuth } from '../context/AuthContext';
import apiService from '../services/api';
import { buildAndDownloadSignedCKDPDF } from '../utils/assessmentPdf';
...
export default function CKDReportPage({ language }) {
  ...
  const handleShare = async () => {
    ...
    try {
      const result = await apiService.shareCKDAssessment(assessment.id);
  ...
  const isCKDPositive = (assessment.prediction || '').toLowerCase() === 'ckd';
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 pt-32 pb-24">
      <div className="mb-6">
        <button
          type="button"
          onClick={() => navigate(`${ROUTES.DASHBOARD}?tab=assessments&type=ckd`)}
