...
const Footer = ({ language }) => {
  ...
  const tools = [
    { label: isAr ? 'تقييم المخاطر' : isTr ? 'Risk Değerlendirmesi' : 'Risk Assessment', path: ROUTES.TEST, icon: Activity },
    { label: isAr ? 'محادثة ذكية' : isTr ? 'Yapay Zeka Sohbet' : 'AI Chat', path: ROUTES.CHAT, icon: MessageSquare },
    { label: isAr ? 'مساعد صوتي' : isTr ? 'Sesli Asistan' : 'Voice Assistant', path: ROUTES.VOICE_CHAT, icon: Mic },
    { label: isAr ? 'خطة غذائية' : isTr ? 'Diyet Planı' : 'Diet Plan', path: ROUTES.DIET_PLAN, icon: Apple },
    { label: isAr ? 'فحص الأعراض' : isTr ? 'Belirti Kontrolü' : 'Symptom Checker', path: ROUTES.SYMPTOM_CHECKER, icon: AlertTriangle },
    { label: isAr ? 'المستشفيات' : isTr ? 'Hastaneler' : 'Hospitals', path: ROUTES.HOSPITALS, icon: MapPin },
    { label: isAr ? 'التصوير الدماغي' : isTr ? 'Beyin MR' : 'Brain MRI', path: ROUTES.BRAIN_MRI, icon: Brain },
    { label: isAr ? 'تحقق من التقرير' : isTr ? 'Rapor Doğrula' : 'Verify Report', path: ROUTES.VERIFY, icon: Shield, dataTour: 'footer-verify' },
  ];

  const company = [
    { label: isAr ? 'الأسعار' : isTr ? 'Fiyatlandırma' : 'Pricing', path: ROUTES.PRICING },
    { label: isAr ? 'تسجيل الدخول' : isTr ? 'Giriş Yap' : 'Sign In', path: ROUTES.LOGIN },
    { label: isAr ? 'إنشاء حساب' : isTr ? 'Kayıt Ol' : 'Register', path: ROUTES.REGISTER },
  ];
    ...
    <footer className="relative z-10" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
          ...
          <div className="md:col-span-4 space-y-6">
            ...
            <p className="text-sm text-gray-500 leading-relaxed max-w-xs">
              {isAr
                ? 'منصة صحية بالذكاء الاصطناعي. اكتشف الأمراض المزمنة مبكراً وتحكّم بصحتك.'
                : isTr
            ...
            <button onClick={() => navigate(ROUTES.REGISTER)}
              ...
              {isAr ? 'ابدأ مجاناً' : isTr ? 'Ücretsiz başlayın' : 'Get started free'}
          ...
          <div className="md:col-span-4">
            <h4 className="text-[11px] font-extrabold text-gray-500 mb-5 uppercase tracking-[0.2em]">
              {isAr ? 'الأدوات' : isTr ? 'Araçlar' : 'Tools'}
            </h4>
          ...
          <div className="md:col-span-4 grid grid-cols-2 gap-8">
            <div>
              <h4 className="text-[11px] font-extrabold text-gray-500 mb-5 uppercase tracking-[0.2em]">
                {isAr ? 'المنصة' : isTr ? 'Platform' : 'Platform'}
              </h4>
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10 py-6 flex flex-col sm:flex-row items-center justify-between gap-3"
        ...
        <p className="text-[11px] text-gray-700 tracking-wide">
          &copy; {new Date().getFullYear()} Bonus Life AI. {isAr ? 'جميع الحقوق محفوظة.' : isTr ? 'Tüm hakları saklıdır.' : 'All rights reserved.'}
        </p>
        <div className="flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-violet-500" style={{ animation: 'pulse 2s ease-in-out infinite' }} />
          <span className="text-[11px] text-gray-700">{isAr ? 'مدعوم بالذكاء الاصطناعي' : isTr ? 'Yapay zeka ile çalışıyor' : 'Powered by AI'}</span>
        </div>
      </div>
    </footer>
  );
